...
def iterate(snp_data, pheno,n_iter,threshs,thresh_LD,nb_pc,repertoire):
    ...
    for iteration in range(n_iter):
        ...
        # Classe les SNPs par I scores et P-values 
        sorted_snp_pval=np.argsort(low_ld_res.P)
        sorted_snp_iscore=np.argsort(-i_scores_classification)
        sorted_snps_low_ld = np.argsort(np.argsort(sorted_snp_pval)+np.argsort(sorted_snp_iscore))
...
if __name__ == '__main__':
    ...
    step = 10
    ############
    repertoire="/media/vcabeliNFS2/safia/"+str(sys.argv[1])+"_combined"
