...
def iterate(snp_data, pheno,n_iter,threshs,thresh_LD,nb_pc,repertoire):
    ...
    for iteration in range(n_iter):
        ...
        ##################LD-score
        cmd_CM ='''/media/vcabeliNFS2/safia/bin/plink --bfile {} --cm-map {} --make-bed --out {}'''.format(repertoire+"/training_set","/media/vcabeliNFS2/safia/1000GP_Phase3/genetic_map_@_combined_b37.txt",repertoire+"/training_CM")
        print cmd_CM
        p = subprocess.Popen(cmd_CM, shell=True)
        p.wait()

        cmd_ldscore = "/media/vcabeliNFS2/safia/bin/ldsc.py --bfile {} --l2 --ld-wind-cm 1 --out {}".format(repertoire+"/training_CM",repertoire+"/training_LD")
        print cmd_ldscore
        p = subprocess.Popen(cmd_ldscore, shell=True)
        p.wait()

        cmd_unzip="gunzip {}.l2.ldscore.gz".format(repertoire+"/training_LD")
        p = subprocess.Popen(cmd_unzip, shell=True)
        p.wait()

        LD_scores = pd.read_table("{}.l2.ldscore".format(repertoire+"/training_LD"), delim_whitespace=True)

        sorted_snps_ld_score = np.argsort(LD_scores.L2)
        
        sorted_snps_low_ld = np.argsort(np.argsort(sorted_snps_ld_score)+np.argsort(sorted_snps_iscore))
        
        
        ################# prunnig: SNPs indépendantes
        ...
        sorted_snps_low_ld = np.array([i for i in sorted_snps_low_ld if snp_data.col[i] in pruned_snps])
...
if __name__ == '__main__':
    ...
    step = 10
    ############
    repertoire="/media/vcabeliNFS2/safia/"+str(sys.argv[1])+"_combined_2"
